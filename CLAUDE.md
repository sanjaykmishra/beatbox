...
5. **Reports lock when published.** The lifecycle is `draft → processing → ready → published`, with `processing → failed` recoverable via re-generate. `draft`, `ready`, and `failed` are editable (Add URLs / Retry / Patch / Edit summary / Generate); `processing` rejects edits to avoid racing the renderer; `published` rejects every mutation and refuses delete. Apply via `ReportMutationGuard.assertEditable(report, …)` on every endpoint that mutates a report or its child rows. Publishing is the explicit user action that locks the report; in multi-person workspaces (≥2 active members) the report's creator cannot self-publish — only another team member can. Sharing requires `published`. See `docs/03-data-model.md` and the V013 migration.
